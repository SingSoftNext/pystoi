...
def remove_silent_frames(x, y, dyn_range, framelen, hop):
    ...
    use_NOLA = False
    if use_NOLA:
        print(f"Using NOLA? {use_NOLA}")
    if use_NOLA:
        # Add boundary and padding
        nshort = (len(x) - framelen) % hop
        pad = np.zeros(hop - nshort, dtype=x.dtype)
        x2 = np.concatenate((x, pad))
        y2 = np.concatenate((y, pad))
        nbdry = framelen // 2
        bdry = np.zeros(nbdry, dtype=x.dtype)
        x2 = np.concatenate((bdry, x2, bdry))
        y2 = np.concatenate((bdry, y2, bdry))

        # Create silence mask
        w = scipy.hanning(framelen + 2)[1:-1]
        nframes = (len(x2) - framelen) // hop + 1
        mask = np.array(
            [
                20 * np.log10(
                    np.linalg.norm(w * x2[i * hop:i * hop + framelen]) + EPS
                )
        ...
        nframes = (len(x2) - framelen + hop) // hop
        d = np.zeros_like(x2)  # normalization factors (denominator)
        for t in range(nframes):
            d[t * hop:t * hop + framelen] += w

        # Remove silent frames
        count = 0
        x_sil = np.zeros(x2.shape)
        y_sil = np.zeros(y2.shape)
        for i in range(mask.shape[0]):
            if mask[i]:
                sil_ind = range(count * hop, count * hop + framelen)
                ind = range(i * hop, i * hop + framelen)
                x_sil[sil_ind] += x2[ind] * w / d[ind]
                y_sil[sil_ind] += y2[ind] * w / d[ind]
                count += 1
        ...
        # Remove boundaries
        #TODO Are the boundaries still there in the *_sil arrays?
        x_sil = x_sil[nbdry:-nbdry]
        y_sil = y_sil[nbdry:-nbdry]

        # Remove padding
        #TODO Is the padding still there in the *_sil arrays?
        if nshort > 0:
            x_sil = x_sil[:-len(pad)]
            y_sil = y_sil[:-len(pad)]
    else:
